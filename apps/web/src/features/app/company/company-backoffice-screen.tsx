                ...
                <Stack spacing={2}>
                  ...
                  <Stack spacing={2} component="form" onSubmit={(event) => {
                    ...
                    void saveSection<{
                      ...
                      const nextData = { ...data, appointmentServices: [...data.appointmentServices, payload.appointmentService] };
                      setState({ status: "loaded", data: nextData });
                      hydrate(nextData);
                    });
                  }}>
                      ...
                      <Stack spacing={1.5}>
                        ...
                        <TextField label={`Service name ${service.id}`} value={drafts.appointmentServiceUpdate[service.id]?.name ?? service.name} onChange={(event) => updateDrafts((current) => ({
                          ...
                          appointmentServiceUpdate: {
                            ...current.appointmentServiceUpdate,
                            [service.id]: {
                              ...current.appointmentServiceUpdate[service.id],
                ...
                <Stack spacing={2}>
                  ...
                  <Stack spacing={2} component="form" onSubmit={(event) => {
                    ...
                    void saveSection<{
                      ...
                      const nextData = { ...data, appointmentProviders: [...data.appointmentProviders, payload.appointmentProvider] };
                      setState({ status: "loaded", data: nextData });
                      hydrate(nextData);
                    });
                  }}>
                      ...
                      <Paper key={provider.id} variant="outlined" sx={{ p: 2 }}>
                        <Stack spacing={1.5}>
                          ...
                          <TextField label={`Provider name ${provider.id}`} value={drafts.appointmentProviderUpdate[provider.id]?.displayName ?? provider.displayName} onChange={(event) => updateDrafts((current) => ({
                            ...current,
                            appointmentProviderUpdate: {
                              ...current.appointmentProviderUpdate,
                              [provider.id]: {
                                ...current.appointmentProviderUpdate[provider.id],
                            ...
                            <Grid key={`${provider.id}-${index}`} container spacing={2}>
                              <Grid size={{ xs: 12, md: 4 }}>
                                <TextField label="Day" value={entry.dayOfWeek} onChange={(event) => updateDrafts((current) => ({
                                  ...
                                  providerAvailabilityUpdate: {
                                    ...current.providerAvailabilityUpdate,
                                    [provider.id]: availability.map((item, itemIndex) => itemIndex === index ? { ...item, dayOfWeek: event.target.value } : item)
                                  }
                                }))} select fullWidth>
                                  {days.map((day) => <MenuItem key={day} value={day}>{day}</MenuItem>)}
                                </TextField>
                              ...
                              <Grid size={{ xs: 12, md: 3 }}>
                                <TextField label="Opens" type="time" value={entry.opensAt} onChange={(event) => updateDrafts((current) => ({
                                  ...
                                  providerAvailabilityUpdate: {
                                    ...current.providerAvailabilityUpdate,
                                    [provider.id]: availability.map((item, itemIndex) => itemIndex === index ? { ...item, opensAt: event.target.value } : item)
                                  }
                                }))} InputLabelProps={{ shrink: true }} fullWidth />
                              </Grid>
                              <Grid size={{ xs: 12, md: 3 }}>
                                <TextField label="Closes" type="time" value={entry.closesAt} onChange={(event) => updateDrafts((current) => ({
                                  ...
                                  providerAvailabilityUpdate: {
                                    ...current.providerAvailabilityUpdate,
                                    [provider.id]: availability.map((item, itemIndex) => itemIndex === index ? { ...item, closesAt: event.target.value } : item)
                                  }
                                }))} InputLabelProps={{ shrink: true }} fullWidth />
                              </Grid>
                              <Grid size={{ xs: 12, md: 2 }}>
                                <Button color="error" fullWidth onClick={() => updateDrafts((current) => ({
                                  ...
                                  providerAvailabilityUpdate: {
                                    ...current.providerAvailabilityUpdate,
                                    [provider.id]: availability.filter((_, itemIndex) => itemIndex !== index)
                                  }
                                }))}>Remove</Button>
                              </Grid>
                            </Grid>
                          ...
                          <Stack direction="row" spacing={1}>
                            <Button onClick={() => updateDrafts((current) => ({
                              ...
                              providerAvailabilityUpdate: {
                                ...current.providerAvailabilityUpdate,
                                [provider.id]: [...availability, { dayOfWeek: "MONDAY", opensAt: "09:00", closesAt: "17:00", displayOrder: availability.length }]
                              }
                            }))}>Add window</Button>
                            <Button variant="outlined" onClick={() => void saveSection<{
                              ...
                              const nextData = { ...data, providerSchedules: nextSchedules };
                              setState({ status: "loaded", data: nextData });
                              hydrate(nextData);
